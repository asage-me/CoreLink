...
- [ ] 3.1 First-sample-seeds behavior (spec: *First sample seeds the filter*); `None` until seeded
- [ ] 3.2 Time-corrected update `state + (1 − e^(−Δt/τ))·(x − state)` with injected `Duration` delta (spec: *EMA follows injected time*); tick-gap clamping per design D3
- [ ] 3.3 Unit tests: seeding, one-step math at Δt=τ (60.7 °C case), multi-step convergence, gap clamp

## 4. Fold module

- [ ] 4.1 `FoldRule` enum (`Max` default, `Avg`) reducing a non-empty iterator of smoothed values
- [ ] 4.2 Unit tests: default max (55/78 → 78), avg (55/70 → 62.5), empty input is a caller error (spec: *Fold Rule* scenarios)

## 5. Curve lookup

- [ ] 5.1 Sorted-points storage for ≥2 distinct-temp points (design D5); piecewise-linear interpolation; clamped ends (spec: *Curve Mode* — interior 55 °C → 50 %, above-max clamps)
- [ ] 5.2 Unit tests for both spec scenarios plus below-min clamping

## 6. PID controller

- [ ] 6.1 Per-profile `Pid` struct: `update(err, Δt) -> f64` output clamped [0,100], gains from config
- [ ] 6.2 Anti-windup back-calculation per design D4 (spec: *Anti-windup under saturation* — 20 s saturated then reversed error, no integral-debt recovery delay)
- [ ] 6.3 Unit tests: proportional-only response with Ki=0 (*Ramps up when hot*), settle behavior, independent integrators across two instances (*Independent integrator per profile*)

## 7. Tripwire state (shared)

- [ ] 7.1 `TripwireState` per source: own EMA, latch with hysteresis clear, dT/dt check disabled until 2 samples (spec: *Tripwire Detection* — threshold trip, 40→80 spike trip, no false first-sample trip)
- [ ] 7.2 Latch persistence: holds while above threshold−hysteresis, clears at or below it (spec: *Tripwire Latching* — 85/82/75 °C sequence)
- [ ] 7.3 `TripwireEval` produced per source per tick: `online` flag + `latched` (last-known when offline) for the profile pipeline (design D2)

## 8. Slew-rate limiter

- [ ] 8.1 `step_toward(position, target, rate, Δt)` with separate up/down rates; position initialized from the injected last-commanded PWM every tick (spec: *upward limited*, *downward slower*, *small correction unimpeded*)
- [ ] 8.2 Unit tests including the 20→40 (up 20 %/s @1 s) and 80→75 (down 5 %/s @1 s) cases
